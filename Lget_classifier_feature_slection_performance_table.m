...
for i_c=1:length(set_classifier)
    for i_f=1:length(set_featureselectioon)
        ...
        for i_m=1:4%for the metric
            if i_m==1
                word_tb((i_c-1)*length(set_featureselectioon)+i_f,i_m)=resultAUC(i_c,i_f).mean;
                word_tb_std((i_c-1)*length(set_featureselectioon)+i_f,i_m)=resultAUC(i_c,i_f).std;
            end
            if i_m==2
                word_tb((i_c-1)*length(set_featureselectioon)+i_f,i_m)=resultACC(i_c,i_f).mean_acc;
                word_tb_std((i_c-1)*length(set_featureselectioon)+i_f,i_m)=resultACC(i_c,i_f).std_acc;
            end
            if i_m==3
                word_tb((i_c-1)*length(set_featureselectioon)+i_f,i_m)=resultACC(i_c,i_f).mean_spec;
                word_tb_std((i_c-1)*length(set_featureselectioon)+i_f,i_m)=resultACC(i_c,i_f).std_spec;                
            end
            if i_m==4
                word_tb((i_c-1)*length(set_featureselectioon)+i_f,i_m)=resultACC(i_c,i_f).mean_sens;
                word_tb_std((i_c-1)*length(set_featureselectioon)+i_f,i_m)=resultACC(i_c,i_f).std_sens;
            end   
        end
    end
end
...
T = array2table(tbl_final,...
    'VariableNames',{'AUC' 'Accuracy' 'Specificity' 'Sensitivity'});
